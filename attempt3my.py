...
def fc(enco):
    flat = Flatten()(enco)
    den = Dense(10, activation='relu')(flat)
    den = BatchNormalization()(den)
    den = Dropout(0.25)
    den = Dense(5, activation='relu')(den)
    den = BatchNormalization()(den)
    den = Dropout(0.25)
    den = Dense(10, activation='relu')(den)
    den = BatchNormalization()(den)
    den = Dropout(0.25)
    den = Dense(5, activation='relu')(den)
    den = BatchNormalization()(den)
...
predict.to_csv('C:/Users/chuda/Bootcamp_project/Deep Learing/Level2_Problem2/Attempt3shoe.csv')
